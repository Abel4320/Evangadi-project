import React, { useContext, useEffect,useState } from 'react';
import QuestionContext from '../../context/QuestionContext';
import { Link, useParams } from 'react-router-dom';
import axios from 'axios'; // Import axios if you haven't already
import  UserContext from '../../context/UserContext'
import './Answer.css'

const Answers = () => {
      const imageUrl = 'data:image/png;base64,'
  const [questionsData, setQuestionData] = useContext(QuestionContext);
  const { question_id } = useParams();
  const [answerText, setAnswerText] = useState('');
  const [userData, setUserData] = useContext(UserContext);
  const [answerData, setanswerData] = useState([]);
 const fetchAnswers = async () => {
    try {
      const response = await axios.get(`http://localhost:5000/api/answer/allanswers`);
      const allAnswers = response.data.data;
      const filteredAnswers = allAnswers.filter((answer) => answer.question_id === parseInt(question_id, 10));
      setanswerData(filteredAnswers);
    } catch (error) {
      console.error('Error fetching answers:', error);
    }
  };
  useEffect(() => {
    // Check if questionsData is empty and fetch data if necessary
    if (questionsData.length === 0) {
      axios.get('http://localhost:5000/api/question/allquestions')
        .then((response) => {
          setQuestionData(response.data.data);
        })
        .catch((error) => {
          console.error('Error fetching questions:', error);
        });
    }
    fetchAnswers()
  } , [questionsData, setQuestionData]); 
  const question = questionsData.find(q => q.question_id.toString() === question_id);
  const answer=answerData.find(a=>a.question_id.toString()=== question_id)
  if (!question) {
    return <div>Loading question...</div>; // Or handle the not-found case
  }
const handleAnswerpost = async (event) => {
  event.preventDefault(); // Prevent page refresh

  try {
    const response = await axios.post('http://localhost:5000/api/answer/postanswer', {
      answer: answerText,
      user_id: userData.user?.id,
      question_id: question.question_id,
      answer_code_block: userData.user?.display_name
    });

    console.log('Answer submitted successfully:', response.data);
    console.log('userData:', userData);
    // Fetch answers again after posting a new answer
    fetchAnswers();
    setAnswerText('');
  } catch (error) {
    console.error('Error posting answer', error);
  }
};
  return (
    <div className=''>
      <div className='ml-[50px] mr-[50px] sm:ml-[100px] sm:mr-[100px] pt-[100px]'>
        <h1 className='font-bold'>Question</h1>
      <p className='text-[20px] font-semibold'>{question.question}</p>
        <p className='text-[15px]'><span className='font-semibold'>Question Description :</span> {question.question_description}</p>
      <p className='text-[12px]'>This Question is Asked By <span className='font-semibold'>{question.tags}</span></p>
      <div>
        <hr className="mx-auto border-t border-gray-300 sm:mr:[100px]" />
        </div>
        <div className='mt-3 '>
         {answerData.length > 0 && (
      <div className='text-[25px] font-semibold'>
        Answer From The Community
              <div>
                 <hr className="mx-auto mt-3 border-t border-gray-300 sm:mr:[100px]" />
        </div>
      </div>
    )}
          <div className='mt-3'>
     {answerData.map((answer) => (
       <div key={answer.answer_id} className='border p-3 mb-3'>
         <div className='flex items-center'>
           <div>
             <img className='w-[50px]' src={imageUrl} alt="Common Image" />
              <h1 className='flex justify-center'>{answer.answer_code_block}</h1> 
            </div>
           <p className='flex-1 pl-5 text-[12px] sm:text-[14px] md:text-[15px] lg:text-[15px] font-semibold'>{answer.answer}</p>
         </div>
            </div>
          ))}
    </div>
        </div>
      </div>
      <div>
         <div className=' justify-center items-center answer-container mt-[50px] mb-[50px]'>
           <h1 className='flex justify-center text-[25px] font-semibold mb-3 pt-5 '>
           Answer this Question
        </h1>
           <div className='flex justify-center mb-3 '>
         <Link to={`/home`}><h1>Go to questions Page</h1></Link>
          </div>
          <div className='flex justify-center'>
             <form className=''  >
              <input
                value={answerText}
            style={{ width: '67vw' ,height:'' ,textAlign: 'left', verticalAlign: 'top' }}
            className={`pl-2 border border-gray-400' rounded-md pb-[170px]`}
            placeholder='Your Answer...'
                type="text"
                  onChange={(e) => setAnswerText(e.target.value)}
            />
            <br />
        {/* Add similar input fields for codeBlock, tags */}
        <button onClick={handleAnswerpost} className='bg-blue-500 hover:bg-orange-500  mt-5 mb-5 rounded-sm h-9 p-2 text-white' type="submit">Post your Answer</button>
      </form>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Answers;
